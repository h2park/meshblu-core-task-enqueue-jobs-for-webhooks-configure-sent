...
describe 'EnqueueJobsForWebhooksConfiguretSent', ->
  beforeEach (done) ->
    @datastore = new Datastore
      database: mongojs('meshblu-core-task-enqueue-jobs-for-webhooks-configure-sent')
      collection: 'devices'
  ...
  beforeEach ->
    client = new RedisNS 'ns', redis.createClient(@redisKey)
    @sut = new EnqueueJobsForWebhooksConfiguretSent {
      datastore:         @datastore
      ...
      context 'when given a valid job where the last hop from does not match the to', ->
        beforeEach (done) ->
          request =
            metadata:
              auth: {uuid: 'subscriber'}
              route: [{type: 'configure.sent', from: 'emitter', to: 'subscriber'}]
              responseId: 'its-electric'
            rawData: '{}'
